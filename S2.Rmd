...
intervention reduces $\Re_t$ below the epidemic threshold (dashed line), the 
susceptible fraction remains above the target (dotted line). Therefore, once the
mobility restrictions are lifted, $\Re_t$ rises above the epidemic threshold, 
leading to a second wave of infections. In contrast, the ideal scenario (*3*) 
with a stringency of 0.62 also drives $\Re_t$ below 1 but allows the susceptible
fraction to reach the intended target. Consequently, when mobility restrictions
are lifted, $\Re_t$ cannot surge to produce a new outbreak.
