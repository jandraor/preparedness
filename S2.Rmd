...
**testing & isolation** and **contact tracing** sub-models.
...
```{r, message = FALSE}
F_matrix           <- read_csv("./Data/TI_F_matrix.csv", show_col_types = FALSE)
...
```{r, message = FALSE}
F_matrix           <- read_csv("./Data/TI_V_matrix.csv", show_col_types = FALSE)
c_names            <- colnames(F_matrix)
c_names[1]         <- ""
colnames(F_matrix) <- c_names
...
\begin{equation}
  ...
  \Re_0 = \frac{\beta  (\gamma  \eta  (\nu -\nu 
   \omega )+\gamma  \kappa  \omega
   +\kappa  \nu  \omega  (1-\theta  \iota^d
   ))}{\gamma  \kappa  \nu }
\end{equation}
...
```{r, fig.cap = "Fig 1. Testing capacity's calibration"}
...
```{r, fig.cap = "Fig 2. Testing capacity's exploration space"}
plot(pars_df, pch = 16, cex = 0.5, col = "steelblue",
     xlab = expression(rho^d), ylab = expression(alpha^d))
```
...
```{r, fig.cap = "Fig 3. Contact-tracing capacity's calibration"}
ggplot(est_obj$sim, aes(time, Q)) +
  geom_line() +
...
```{r, fig.cap = "Fig 4. Contact-tracing capacity's exploration space"}
plot(pars_df, pch = 16, cex = 0.5, col = "steelblue",
     xlab = expression(rho^k), ylab = expression(alpha^k))
```
