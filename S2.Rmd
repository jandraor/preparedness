...
This electronic supplementary material aims to document, in a reproducible 
manner, the simulation results presented in the main text regarding the 
sub-models representing the non-pharmaceutical interventions  (**NPIs**).  
...
```{r}
ct_tv_df <- sim_output |> filter(time == 150) |>
...
```{r, fig.height = 7}
g <- plot_fig_04(tv_df, f_t_df, tv_ct_df, f_ct_df)
...
# Third scenario: TTI + MR, starting at 40
intv_df <- data.frame(par_tau_m = c(0, 0, 40),
...
sim_mob_df <- sd_sensitivity_run(mdl_ct_mob$deSolve_components,
                                 consts_df = select(intv_df, par_tau_m),
...
by the compliance parameter ($\xi$) and the time at which mobility restrictions
...
```{r, fig.cap = "Fig 2. Social distancing exploration space"}
...
```{r}
df_2 <- sim_mr_scn |> mutate(attack_rate = 100 * C/ pop_val) |> 
  filter(time == 270,
         par_xi < 0.8)

df_2 |> arrange(attack_rate) |> head(10) |> 
  select(par_xi, par_tau_m, attack_rate)
```
