...
**no-intervention** and **vaccination** sub-models.

# Within-host profile (no intervention)

## $\Re_0$ analytical expression

We derive an analytical expression for the basic reproduction number ($\Re_0$)
via the next-generation matrix method. This method consists of rewriting the 
rates of infected states in the form of two matrices. One matrix corresponds to 
the rate of appearance of new infections ($V$ matrix), and the other one to the 
rate of transitions between compartments of infected individuals ($F$ matrix). 
The product between $F$ and $V^{-1}$ is known as the next-generation matrix,
whose largest eigenvalue (spectral radius) corresponds to $\Re_0$. We obtain 
the analytical expressions using *Mathematica*.
...
```{r, fig.cap = "Fig 1. Attack rate comparison", fig.height = 4}
...
```{r, fig.cap = "Fig 2. Final size comparison"}
...
```{r, fig.cap = "Fig 3. Vaccination's exploration space"}
plot(consts_df, pch = 16, cex = 0.5, col = "steelblue",
     xlab = expression(rho^v), ylab = expression(tau^v))
```

```{r}
## Simulations
### By transmissibility level
